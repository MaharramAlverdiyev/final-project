          ...
          <div className="product-list" >
            {
              sortedProducts.map((product) => (
                <Link to = {`/product/${product.id}`} style={{ textDecoration: 'none', color: 'inherit' }}><div key={product.id} className="product-card">
                  <img src={product.image} alt={product.name} className="product-image" />
                  <div className="product-detailss">
                    <h3 className="product-namee">{product.name}</h3>
                    <p className="product-brandd">{product.brand}</p>
                    <p className="product-categoryy">Kategoriya: Kişi</p>
                    <p className="product-pricee">Qiymət: {product.price} ₼</p>
                    <p className="product-stockk">Məhsul sayı: {product.stock}</p>
